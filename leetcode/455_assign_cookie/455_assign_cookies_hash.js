...
function convertHashMap(paramArray) {
  let resultHash = {};

  paramArray.forEach((i, index) => {
    if (resultHash[i]) {
...
var findContentChildren = function (g, s) {
  let cookieHashMap = convertHashMap(s);
  console.log(cookieHashMap)

  let resultCount = 0
  g.forEach((item) => {
    if (cookieHashMap[item] && 0 < cookieHashMap[item]) {
      resultCount += 1
    }
  });
